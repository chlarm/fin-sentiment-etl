...
## 4.3.1 การพยากรณ์ความผันผวน — ผลบวกของงานวิจัย

ผลลบใน §4.3 นำไปสู่คำถามว่า ข้อสรุปที่ถูกต้องคือ "ข้อมูลชุดนี้ทำนายอะไรไม่ได้" หรือ "คำถามที่ตั้งไว้เป็นคำถามที่ทำนายไม่ได้" จึงวัดคุณสมบัติของข้อมูลโดยตรง

**ตารางที่ 4.9 ค่าสหสัมพันธ์ในตัวเอง วัดจาก 30 สินทรัพย์ 2006-2026**

| สิ่งที่วัด | ค่าเฉลี่ย | ต่ำสุด |
|---|---|---|
| ผลตอบแทนรายวัน (lag 1) | **−0.053** | −0.289 |
| ความผันผวน 20 วัน (lag 5) | **+0.901** | +0.821 |
| ความผันผวน 20 วัน (lag 20) | +0.504 | +0.215 |

ทิศทางไม่มีความต่อเนื่อง ความผันผวนมีสูงมาก และไม่ใช่ผลจากสินทรัพย์ใดตัวหนึ่ง เพราะค่าต่ำสุดใน 30 รายการยังอยู่ที่ +0.821 ข้อสรุปคือ **Track A ไม่ใช่ความล้มเหลวของแบบจำลอง แต่เป็นคำถามที่ข้อมูลตอบไม่ได้**

**เกณฑ์วัดผล** ค่า R² ของการทำนายความผันผวนจากความผันผวนสูงโดยโครงสร้างอยู่แล้ว จึงไม่ใช้เป็นเกณฑ์ เกณฑ์ที่ใช้คือ **skill เทียบ persistence** ซึ่งเป็นการทำนายฟรีที่ว่า "ช่วงถัดไปเท่ากับค่าปัจจุบัน" และผู้ปฏิบัติงานได้มาโดยไม่มีต้นทุน

**ตารางที่ 4.10 ผลการพยากรณ์ (กันข้อมูลไว้ทดสอบ 25% ท้าย)**

| ช่วงเวลา | R² แบบจำลอง | R² persistence | skill | ค่าคลาดเคลื่อนกลาง (ต่อปี) |
|---|---|---|---|---|
| 5 วัน | 0.534 | 0.408 | **+0.213** | 6.22% เทียบ 7.70% |
| 21 วัน | 0.739 | 0.606 | **+0.339** | 4.67% เทียบ 5.78% |

- **walk-forward ชนะ 5 จาก 5 รอบทั้งสองช่วงเวลา** ครอบคลุม 2010-2026 รวมวิกฤต COVID และตลาดหมี 2022
- **รายสินทรัพย์ชนะ 29 จาก 30** skill มัธยฐาน +0.324 ยกเว้น THBUSD=X
- R² ในกลุ่มตัวอย่าง 0.749 เทียบนอกกลุ่มตัวอย่าง 0.739 ที่ 21 วัน — แทบไม่มี overfitting

**การตรวจสอบก่อนยอมรับผล** เนื่องจากเป็นผลบวกรายการเดียวของงานวิจัย จึงตรวจสามประเด็น

1. **การรั่วของข้อมูล** — ตรวจสูตรหน้าต่างอนาคตทีละตำแหน่ง และยืนยันว่าค่าผิดปกติของวันที่ *t* ไม่เคยปรากฏใน target ของวันที่ *t* เอง
2. **แบบจำลองลอกเส้นฐานหรือไม่** — ตัดตัวแปร `log_vol_20` ออกทั้งหมด R² ยังเท่าเดิมที่ 0.739 จึงไม่ได้ลอก ค่าที่ได้มาจากการพิจารณาความผันผวนหลายช่วงเวลาพร้อมกัน
3. **มาจากสินทรัพย์ส่วนน้อยหรือไม่** — บทเรียนตรงจากกรณี 5 เทียบ 29 สินทรัพย์ ตรวจแล้วชนะ 29/30

**ผลลบภายในผลบวก** ตัวแปรความผันผวนของตลาดโดยรวมไม่ช่วยอะไร (0.739 → 0.740 เมื่อตัดออก) บันทึกไว้ในโค้ดแล้ว

**ขอบเขตการใช้งาน** บอกว่าราคาจะเคลื่อนไหวแรงเพียงใด ไม่ได้บอกทิศทาง ใช้บริหารความเสี่ยงและกำหนดขนาดสถานะได้ แต่ไม่ใช่สัญญาณซื้อขาย และยังไม่ได้ทดสอบผลหลังหักค่าธรรมเนียม

---

> **[รูปที่ 4.6.1]** แท็บ Signal แสดงส่วน Volatility outlook พร้อมตารางหลักฐาน (skill เทียบ persistence)
> 📸 แคปหน้าจอ http://localhost:8000/?tab=predict&ticker=NVDA
...
**ผลลัพธ์เพิ่มเติมนอกเหนือวัตถุประสงค์:** การพยากรณ์ความผันผวน (§4.3.1) เป็นผลบวกรายการเดียวของงานวิจัย เกิดจากการตั้งคำถามใหม่หลังพบว่าคำถามเดิมเป็นคำถามที่ข้อมูลตอบไม่ได้ ชนะเส้นฐาน persistence 5/5 รอบ walk-forward และ 29/30 สินทรัพย์
...
**ข้อสรุปเชิงวิชาการของงานวิจัยนี้มิใช่ "ข้อมูลชุดนี้ทำนายอะไรไม่ได้" แต่เป็นข้อความที่เฉพาะเจาะจงกว่านั้น**

> **ทิศทางของผลตอบแทนทำนายไม่ได้ แต่ขนาดของการเคลื่อนไหวทำนายได้**

| Track | คำถาม | ผล |
|---|---|---|
| A | sentiment ข่าวทำนายทิศทางระยะสั้นได้หรือไม่ | **ไม่ได้** — AUC ลดลงทุกช่วงเวลา, ชนะเส้นฐาน 0/3 รอบ |
| B | งบการเงินสัมพันธ์กับผลตอบแทนระยะกลาง-ยาวหรือไม่ | **ไม่ได้** — ไม่รอดการตรวจสอบทั้งสามชั้น |
| C | ความผันผวนพยากรณ์ได้หรือไม่ | **ได้** — skill +0.213/+0.339, ชนะ 5/5 รอบ, 29/30 สินทรัพย์ |

ผลลบของ Track A และ B สอดคล้องกับสมมติฐานตลาดมีประสิทธิภาพในรูปแบบกึ่งเข้ม ซึ่งระบุว่าข้อมูลสาธารณะถูกสะท้อนในราคาไปแล้ว ส่วนผลบวกของ Track C ไม่ขัดแย้งกับสมมติฐานดังกล่าว เพราะการพยากรณ์ความผันผวนไม่ได้ให้ข้อได้เปรียบในการทำกำไรจากทิศทางราคา แต่เป็นข้อมูลด้านความเสี่ยง

**Track C ยังทำหน้าที่เป็น positive control ทางระเบียบวิธี** ผลลบเพียงอย่างเดียวอาจถูกตีความว่าเกิดจากระบบวัดที่บกพร่อง การที่ระบบเดียวกัน ด้วยมาตรฐานการตรวจสอบเดียวกัน สามารถตรวจพบสัญญาณเมื่อสัญญาณมีอยู่จริง เป็นหลักฐานว่าผลลบของ Track A และ B เกิดจากการที่ไม่มีสัญญาณ ไม่ใช่จากการที่ระบบตรวจไม่พบ

การนำเสนอผลลัพธ์เชิงลบอย่างตรงไปตรงมา พร้อมบันทึกกระบวนการที่นำไปสู่ข้อสรุป รวมถึงข้อสรุปที่เคยยอมรับแล้วภายหลังถูกถอน ถือเป็นส่วนหนึ่งของคุณค่าทางวิชาการ เนื่องจากแสดงให้เห็นว่าความสัมพันธ์ที่ดูมีนัยสำคัญในข้อมูลทางการเงินเกิดจากค่าผิดปกติเพียงไม่กี่รายการ จากขนาดตัวอย่างที่ถูกนับเกินจริง หรือจากกลุ่มตัวอย่างที่เล็กเกินไป ได้บ่อยเพียงใด

คุณูปการหลักของโครงการจึงเป็น **โครงสร้างพื้นฐานข้อมูลและระเบียบวิธีตรวจสอบที่แยกแยะได้ว่าคำถามใดที่ข้อมูลตอบได้และคำถามใดที่ตอบไม่ได้** ซึ่งเป็นเงื่อนไขจำเป็นสำหรับการศึกษาต่อยอดใดๆ ในหัวข้อนี้
