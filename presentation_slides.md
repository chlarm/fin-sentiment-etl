...
## 🎯 Slide 13b — แล้วอะไรที่ทำนายได้? (ผลบวกของงานวิจัย)

### เนื้อหาบน Slide:

**❓ ถ้าทิศทางทำนายไม่ได้ แปลว่าข้อมูลนี้ไร้ประโยชน์หรือเปล่า**

**ไม่ใช่ — คำถามต่างหากที่ผิด** วัดคุณสมบัติข้อมูลตรงๆ จาก 30 สินทรัพย์ 2006-2026:

| สิ่งที่วัด | autocorrelation |
|---|---|
| **ทิศทาง**ผลตอบแทน (lag 1) | **−0.053** ≈ ศูนย์ |
| **ความผันผวน** 20 วัน (lag 5) | **+0.901** |
| ความผันผวน 20 วัน (lag 20) | +0.504 |

ค่าต่ำสุดใน 30 สินทรัพย์ยังอยู่ที่ **+0.821** — ไม่ใช่ผลจากตัวใดตัวหนึ่ง

**📊 ผลการพยากรณ์ความผันผวน**

| ช่วงเวลา | R² โมเดล | R² ทำนายฟรี | **skill** | ค่าคลาดเคลื่อน |
|---|---|---|---|---|
| 5 วัน | 0.534 | 0.408 | **+0.213** | 6.22% vs 7.70% |
| 21 วัน | 0.739 | 0.606 | **+0.339** | 4.67% vs 5.78% |

- ✅ **ชนะ 5/5 folds** ทั้งสองช่วงเวลา (2010-2026 รวม COVID + ตลาดหมี 2022)
- ✅ **ชนะ 29/30 สินทรัพย์**

### 📸 รูปที่ต้องใส่:
- Screenshot แท็บ Signal ส่วน **Volatility outlook** พร้อมตารางหลักฐาน
- 📸 `http://localhost:8000/?tab=predict&ticker=NVDA`

### 🎙️ Speaker Notes:
> "สไลด์ที่แล้วเป็นผลลบ สไลด์นี้คือผลบวกครับ
>
> หลังจากพบว่าทิศทางทำนายไม่ได้ ผมตั้งคำถามกับตัวเองว่า สรุปว่าข้อมูลชุดนี้ทำนายอะไรไม่ได้เลย หรือว่า**คำถามที่ผมตั้งไว้ต่างหากที่เป็นคำถามที่ทำนายไม่ได้** ซึ่งเป็นคนละเรื่องกัน
>
> ผมจึงไปวัดคุณสมบัติของข้อมูลตรงๆ ผลคือทิศทางของผลตอบแทนมี autocorrelation ติดลบเล็กน้อยคือแทบไม่มีความต่อเนื่องเลย แต่**ความผันผวนมีถึง 0.901** และตัวที่ต่ำสุดใน 30 สินทรัพย์ก็ยังอยู่ที่ 0.821 ปรากฏการณ์นี้เรียกว่า volatility clustering เป็นข้อเท็จจริงเชิงประจักษ์ที่ทำซ้ำได้มากที่สุดอันหนึ่งในการเงิน
>
> **จุดสำคัญที่สุดของสไลด์นี้คือเกณฑ์วัดผลครับ** — ผมไม่ได้วัดด้วย R² เพราะการทำนายความผันผวนจากความผันผวนได้ R² สูงโดยธรรมชาติอยู่แล้ว รายงานไปก็ไม่มีความหมาย
>
> เกณฑ์ที่ผมใช้คือต้องชนะ **persistence** ซึ่งเป็นการทำนายฟรีที่ว่า 'สัปดาห์หน้าผันผวนเท่าสัปดาห์นี้' ใครก็ทำได้โดยไม่ต้องมีโมเดล ถ้าชนะไม่ได้ โมเดลก็ไม่มีเหตุผลที่จะมีอยู่
>
> ผลคือชนะทั้ง 5 folds ทั้งสองช่วงเวลา และชนะ 29 จาก 30 สินทรัพย์ครับ"

> **💡 คำถามที่กรรมการน่าจะถามสไลด์นี้**
> - *"R² 0.739 สูงเกินจริงหรือเปล่า"* → สูงโดยโครงสร้าง เพราะทำนายความผันผวนจากความผันผวน ผมจึงไม่ใช้เป็นเกณฑ์ ตัวเลขที่ควรดูคือ skill +0.339 ซึ่งเทียบกับการทำนายฟรี
> - *"มั่นใจได้ยังไงว่าไม่มีข้อมูลอนาคตรั่ว"* → ตรวจสูตรหน้าต่างทีละตำแหน่ง และยืนยันด้วยข้อมูลสังเคราะห์ว่าค่าผิดปกติของวันที่ t ไม่เคยปรากฏใน target ของวันที่ t เอง มีเทสต์อัตโนมัติล็อกไว้
> - *"โมเดลแค่ลอกค่า persistence มาหรือเปล่า"* → ตัดตัวแปรนั้นออกทั้งหมด R² ยังเท่าเดิมที่ 0.739 ค่าที่ได้มาจากการดูความผันผวนหลายช่วงเวลาพร้อมกัน
> - *"ผลบวกนี้ขัดกับผลลบก่อนหน้าไหม"* → ไม่ขัด และยังช่วยยืนยันกันด้วย เพราะแสดงว่าระบบวัดของผมตรวจเจอสัญญาณได้เมื่อมีสัญญาณจริง ผลลบจึงมาจากการไม่มีสัญญาณ ไม่ใช่ระบบวัดบกพร่อง

---

## 🎯 Slide 13c — คำถามเตรียมรับ (ต่อจาก Slide 13)
...
| Track | คำถาม | ผล |
|---|---|---|
| A | sentiment ทำนายทิศทางได้ไหม | ❌ ไม่ได้ (0/3 folds) |
| B | งบการเงินสัมพันธ์กับผลตอบแทนไหม | ❌ ไม่ได้ (ไม่รอด 3 ชั้น) |
| C | ความผันผวนพยากรณ์ได้ไหม | ✅ **ได้ (5/5 folds, 29/30 สินทรัพย์)** |

ผลลบสอดคล้องกับสมมติฐานตลาดมีประสิทธิภาพรูปแบบกึ่งเข้ม ส่วนผลบวกไม่ขัดแย้ง เพราะความผันผวนเป็นข้อมูลความเสี่ยง ไม่ใช่ข้อได้เปรียบด้านทิศทางราคา

**Track C ทำหน้าที่เป็น positive control** — พิสูจน์ว่าระบบวัดตรวจเจอสัญญาณได้เมื่อมีสัญญาณจริง ผลลบจึงมาจากการไม่มีสัญญาณ ไม่ใช่ระบบบกพร่อง
